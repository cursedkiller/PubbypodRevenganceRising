...
/mob/living/simple_animal/god/ClickOn(atom/A, params)
	...
	if(istype(A, /obj/structure/divine/shrine))
		...
		return
	if(istype(A, /obj/structure/divine/fountain))
		var/obj/structure/divine/fountain/F = A
		if(!F.deity)
			to_chat(src, span_warning("This fountain is not connected to a deity!"))
			return
		if(team_colour != F.deity.team_colour)
			to_chat(src, span_warning("This fountain belongs to a different deity!"))
			return
		if(F.mode_cooldown > world.time)
			to_chat(src, span_warning("The fountain's power is still settling. Wait [round((F.mode_cooldown - world.time)/10)] seconds."))
			return
		if(F.active_mode == "life")
			F.active_mode = "death"
			F.refill_reagents()
			F.visible_message(span_warning("[F]'s waters darken, swirling with deathly energy."))
		else
			F.active_mode = "life"
			F.refill_reagents()
			F.visible_message(span_notice("[F]'s waters shimmer with a pure, life-giving light."))
